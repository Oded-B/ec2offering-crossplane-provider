...
import (
	...
	"github.com/Oded-B/ec2offering-crossplane-provider/apis"
	"github.com/Oded-B/ec2offering-crossplane-provider/apis/v1alpha1"
	ec2offering "github.com/Oded-B/ec2offering-crossplane-provider/internal/controller"
	"github.com/Oded-B/ec2offering-crossplane-provider/internal/features"
	"github.com/Oded-B/ec2offering-crossplane-provider/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "Ec2Offering support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-ec2offering"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-ec2offering",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Ec2Offering APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-ec2offering:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(ec2offering.Setup(mgr, o), "Cannot setup Ec2Offering controllers")
